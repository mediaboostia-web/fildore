...
export function ModelForm({ item }: ModelFormProps) {
  ...
  // Sert au bouton « Annuler » : sans saisie, on sort sans rien demander.
  const isDirty =
    name !== (item?.name ?? "") ||
    category !== (item?.category ?? "robe") ||
    garmentType !== (item?.garmentType ?? "robe") ||
    description !== (item?.description ?? "") ||
    indicativePrice !== (item?.indicativePrice ?? 0) ||
    estimatedDelayDays !== (item?.estimatedDelayDays ?? 7) ||
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      ...
      if (res.success && res.data) {
        ...
        // `revalidatePath` a déjà invalidé les pages côté serveur : un
        // `router.refresh()` après le `push` ne ferait qu'un second rendu,
        // visible comme un temps d'attente supplémentaire.
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4" noValidate>
      ...
      {/* Le formulaire proposait quatre photos de banque d'images en dur. Un
          atelier doit reconnaître ses propres créations dans son catalogue :
          la photo vient désormais de son téléphone. */}
      <PhotoField
        label="Photo du modèle"
        value={imageUrl}
        onChange={setImageUrl}
        error={fieldErrors.imageUrl?.[0]}
        hint="Photographiez la création à la lumière du jour. La photo est réduite automatiquement avant l'envoi."
      />
      ...
      <div className="flex flex-col-reverse gap-2 border-t border-border pt-4 sm:flex-row sm:items-center sm:justify-between">
        <CancelFormButton
          href={isEditing ? `/modeles/${item.id}` : "/modeles"}
          isDirty={isDirty}
          disabled={isPending}
          description="Le modèle ne sera pas enregistré : le nom, la description, le prix et la photo saisis seront perdus."
        />
        <Button
          type="submit"
          fullWidth="mobile"
