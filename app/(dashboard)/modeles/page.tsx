...
function ModelPlaceholder({ category, name }: { category: CatalogCategory; name: string }) {
  ...
  return (
    <div
      role="img"
      aria-label={name}
      className="relative flex aspect-4/3 w-full items-center justify-center rounded-t-lg border-b border-border transition-transform group-hover:scale-102"
      ...
      {filteredItems.length === 0 ? (
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {filteredItems.map((item) => (
            <div
              key={item.id}
              className="group flex flex-col justify-between rounded-lg border border-border bg-surface shadow-sm transition-shadow hover:shadow-md overflow-hidden"
            >
              <div>
                <ModelPlaceholder category={item.category} name={item.name} />
                ...
                <div className="p-4 space-y-2">
                  ...
                  <div className="flex flex-wrap gap-1 pt-1">
                    {item.tags.map((tag) => (
                      <span
                        key={tag}
                        className="inline-flex items-center rounded bg-canvas px-2 py-0.5 text-[11px] text-text-subtle border border-border"
                      >
                        {tag}
                      </span>
                    ))}
              ...
              <div className="border-t border-border bg-canvas/40 p-4 flex items-center justify-between">
                <div>
                  <span className="text-xs text-text-muted block">Prix indicatif</span>
